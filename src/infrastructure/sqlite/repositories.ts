...
export class SqliteCodexSessionRepository implements CodexSessionRepository {
  ...
  async findById(id: string): Promise<CodexSessionAggregate | null> { return this.find("id = ?", id); }
  async findByThreadId(threadId: string): Promise<CodexSessionAggregate | null> { return this.find("thread_id = ?", threadId); }
  ...
  private async find(where: string, value: unknown): Promise<CodexSessionAggregate | null> {
    const row = this.db.connection.prepare(`SELECT * FROM codex_sessions WHERE ${where}`).get(value) as Row | undefined;
    return row ? this.map(row) : null;
  }

  private async listRows(suffix: string, ...values: unknown[]): Promise<readonly CodexSessionAggregate[]> {
...
export class SqliteCodexTurnRepository implements CodexTurnRepository {
  ...
  async listBySessionId(sessionId: string): Promise<readonly CodexTurn[]> {
    return (this.db.connection.prepare("SELECT * FROM codex_turns WHERE session_id = ? ORDER BY sequence").all(sessionId) as Row[]).map(mapTurn);
  }
}

export class SqliteHookEventRepository implements HookEventRepository {
  constructor(private readonly db: SynapseDatabase) {}
  async exists(key: string): Promise<boolean> { return this.db.connection.prepare("SELECT 1 FROM hook_events WHERE deduplication_key = ?").get(key) !== undefined; }
...
export class SqliteSummaryProfileRepository implements SummaryProfileRepository {
  ...
  async list(): Promise<readonly SummaryProfile[]> {
    return (this.db.connection.prepare("SELECT * FROM summary_profiles ORDER BY is_default DESC, name").all() as Row[]).map(mapProfile);
  }
  ...
  async delete(id: string): Promise<void> { this.db.connection.prepare("DELETE FROM summary_profiles WHERE id = ? AND id != 'builtin-task-retrospective'").run(id); }
}
...
export class SqliteSummaryDocumentRepository implements SummaryDocumentRepository {
  constructor(private readonly db: SynapseDatabase) {}
  async findById(id: string): Promise<SummaryDocumentAggregate | null> { return this.find("d.id = ?", id); }
  async findLatestBySessionId(id: string): Promise<SummaryDocumentAggregate | null> { return this.find("d.session_id = ? ORDER BY d.updated_at DESC", id); }
  ...
  private async find(where: string, value: unknown): Promise<SummaryDocumentAggregate | null> {
...
export class SqliteOutboxRepository implements OutboxRepository {
  ...
  async listPending(kind: OutboxMessage["kind"], limit: number): Promise<readonly OutboxMessage[]> {
    return (this.db.connection.prepare("SELECT * FROM outbox WHERE kind = ? AND processed_at IS NULL AND attempts = 0 ORDER BY created_at LIMIT ?").all(kind, limit) as Row[]).map((row) => ({
  ...
  async markProcessed(id: string, at: string): Promise<void> { this.db.connection.prepare("UPDATE outbox SET processed_at = ?, last_error = NULL WHERE id = ?").run(at, id); }
  async markFailed(id: string, error: string): Promise<void> { this.db.connection.prepare("UPDATE outbox SET attempts = attempts + 1, last_error = ? WHERE id = ?").run(error, id); }
  async markAggregateProcessed(kind: OutboxMessage["kind"], aggregateId: string, at: string): Promise<void> {
    this.db.connection.prepare("UPDATE outbox SET processed_at = ?, last_error = NULL WHERE kind = ? AND aggregate_id = ? AND processed_at IS NULL").run(at, kind, aggregateId);
  }
}

const defaultSettings: ApplicationSettings = {
  codexBinaryPath: null, summaryModel: null, syncNotesByDefault: false, notesAccount: null,
  notesFolder: "Synapse", widgetVisible: true, widgetPositions: {},
};
