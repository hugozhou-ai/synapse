...
export function SettingsPage() {
  ...
  const hookAction = async (install: boolean) => { setSaving(true); setError(null); try { setHooks(install ? await window.synapse.hooks.install() : await window.synapse.hooks.uninstall()); } catch (reason) { setError(messageOf(reason)); } finally { setSaving(false); } };
