...
import { BookOpen, Code2, NotebookPen, Plus, ShieldCheck, Sparkles, Trash2, X } from "lucide-react";
...
export function SettingsPage() {
  ...
  return <div className="page settings-page">
    ...
    <section className="settings-section"><div className="settings-title"><div className="setting-icon accent"><Sparkles size={18} /></div><div><h2>Codex Hook</h2><p>SessionStart、UserPromptSubmit 与 Stop；安装操作会备份并原子合并现有配置。</p></div><span className={`health ${hooks?.trusted ? "good" : "warn"}`}>{hooks?.trusted ? "已启用" : hooks?.installed ? "待信任" : "未安装"}</span></div><div className="hook-paths"><code>{hooks?.relayPath}</code><code>{hooks?.configPath}</code><code>运行日志：~/Library/Application Support/Synapse/logs/synapse.log</code></div>{hooks?.message && (hooks.installed ? <InfoBanner message={hooks.message} /> : <ErrorBanner message={hooks.message} />)}{hooks && hooks.trustStates.length > 0 && <div className="trust-list">{hooks.trustStates.map((state) => <span key={state.cwd}><code>{shortPath(state.cwd)}</code><em className={state.status}>{state.status}</em></span>)}</div>}<div className="row-actions">{pendingHooks.length > 0 && <button className="primary" disabled={saving} onClick={() => setReviewingHookTrust(true)}><ShieldCheck size={14} />检查并信任</button>}{hooks?.installed ? <button className="danger" disabled={saving} onClick={() => hookAction(false)}><Trash2 size={14} />卸载自有 Hook</button> : <button className="primary" disabled={saving} onClick={() => hookAction(true)}><Plus size={14} />{hooks?.message?.includes("安装不完整") ? "修复安装" : "安装 Hook"}</button>}</div></section>
